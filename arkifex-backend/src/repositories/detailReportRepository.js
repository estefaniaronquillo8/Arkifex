      ...
      if (!project) {
        ...
        const PlanningResults = await ProjectPlanning.findAll({
            ...
            attributes: [
              ...
              [sequelize.fn('SUM', sequelize.col('ResourceAssignments.actualCost')), 'ActualTotalCostOfTask'],
              [sequelize.fn('SUM', sequelize.col('ResourceAssignments.estimatedCost')), 'EstimatedTotalCostOfTask'],
              [sequelize.literal('COUNT(ResourceAssignments.resourceId)'), 'ResourcesCount'],
          ...
          const ResourceResults = await ResourceAssignment.findAll({
            ...
            attributes: [
              ...
              [sequelize.fn('SUM', sequelize.col('ResourceAssignment.actualCost')), 'ActualCostOfResource'],
              [sequelize.fn('SUM', sequelize.col('ResourceAssignment.estimatedCost')), 'EstimatedCostOfResource'],
              [sequelize.literal('COUNT(ResourceAssignment.resourceId)'), 'ResourcesCount'],
              [sequelize.literal('SUM(actualCost) - SUM(estimatedCost)'), 'CostVariance'],
            ],
            include: [
              {
            ...
            group: ['ProjectPlanning.id', 'ProjectPlanning.name', 'Resource.name','Resource.type'],
            ...
            const mappedResults = ResourceResults.map(async (result) => {
              ...
              const UnitaryEstimatedTotalCostOfTask =  EstimatedCostOfResource/ResourcesCount;
              const costDifference = UnitaryActualCostOfTask-UnitaryEstimatedTotalCostOfTask;
        ...
        return { status:200,message: 'DetailReportPlanning Created Successfully'};      
